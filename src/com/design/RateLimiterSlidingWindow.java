...
public class RateLimiterSlidingWindow {
    ...
    public RateLimiterSlidingWindow(int qps) {
        ...
        for (int i = 0; i < windowArray.length; i++) {
            windowArray[i] = new WindowInfo(currentTimeMillis + i * (windowSize / windowCount), new AtomicInteger(0));
        }
    }
     ...
     * 3. 当前 QPS 是否超过限制
     *
     * 滑动窗口是在计数器限流的固定窗口的基础上，把固定窗口划分为多个小窗口，
     * 这些小窗口构成一个环，每个小窗口分别计数，而这些小窗口计数的总和不能超过总的限制，
     * 从而保证限流。
     * @return
     */
