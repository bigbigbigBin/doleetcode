...
//    }
    ...
    // 求最值，且能看出递推规则的，一般用动态规划
    // dp[i][j] 表示以nums1[i]为子数组的结尾，nums2[j]为子数组的结尾, 这俩个子数组所组成的公共子数组的长度
    ...
    public int findLength2(int[] nums1, int[] nums2) {
        ...
        /**
         * 也就是说，严格定义dp[i][j] 表示以nums1[i]为子数组的结尾，nums2[j]为子数组的结尾, 这俩个子数组所组成的公共子数组的长度
         * 那么递推公式，就是dp[i][j] = dp[i-1][j-1] + 1， if nums1[i] == nums2[j]
         *
...
//                    System.out.println("i = " + i + ", j = " + j);
    ...
    public int findLength3(int[] nums1, int[] nums2) {
        /**
         * 上一个版本的动态规划，初始化比较麻烦，自己需要额外写两层循环初始化
         * 那么怎么优化呢？
         * 这个数组的含义，既然是自己定义的，那么我可否定义成其他含义呢？
         * dp[i][j]，定义成该位置存储的是：以A[i-1] 和 B[j-1]结尾的 最长相同公共子串长度
         * 那么 dp[i][j] = dp[i-1][j-1] + 1， if( A[i-1]== B[j-1] )
         *          如果A[i-1]和B[j-1]相等，则以A[i-1]、B[j-1]结尾的最长公共子串的长度等于以A[i-2]、B[j-2]为结尾的公共子串的长度 + 1；
         *          这个+1就是指的A[i-1] ==B[j-1]时所带来的增量长度， 以A[i-2]、B[j-2]为结尾的的公共子串的长度所存放的位置在dp[i-1][j-1];
         */
